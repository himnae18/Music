...
현재: #${tag}`, tag) || "";
  ...
  function bindPlaylistVideoAdd(selected) {
    ...
    const run = async () => {
      ...
      try {
        const result = await S.addVideoToCustomPlaylist?.({ ytUrl, playlist: selected, tags: extraTags });
      ...
      playlistHead.innerHTML = `
        ...
        <div class="tag-player-add-video">
          ...
          <div class="tag-player-add-row playlist-only-add-row">
            <input id="tagPlayerAddUrl" placeholder="유튜브 영상 링크" />
